...
void CPFA_controller::UpdateLocalPheromoneList() {
	if((SimulationTick() % (SimulationTicksPerSecond() / 2)) != 0) return;

	argos::Real currentTime =
		(argos::Real)SimulationTick() / (argos::Real)SimulationTicksPerSecond();

	std::vector<Pheromone> activePheromones;

	for(size_t i = 0; i < LocalPheromoneList.size(); i++) {
		LocalPheromoneList[i].Update(currentTime);

		if(LocalPheromoneList[i].IsActive()) {
			activePheromones.push_back(LocalPheromoneList[i]);
		}
	}

	LocalPheromoneList = activePheromones;
}
...
void CPFA_controller::SharePheromonesLocally() {
	...
	for(argos::CSpace::TMapPerType::iterator it = footbots.begin();
		...
		CPFA_controller& other =
			dynamic_cast<CPFA_controller&>(
